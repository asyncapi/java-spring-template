...
public class {{schemaName | camelCase | upperFirst}} {
    ...
    {%- if prop.type() == 'number' and (prop.format() == 'float' or prop.format() == 'double') %}
        {%- if prop.minimum() %}@DecimalMin({{prop.minimum()}}, inclusive = true){% endif %}{%- if prop.exclusiveMinimum() %}@DecimalMin({{prop.exclusiveMinimum() + 1}}, inclusive = false){% endif %}
        {%- if prop.maximum() %}@DecimalMax({{prop.maximum()}}, inclusive = true){% endif %}{%- if prop.exclusiveMaximum() %}@DecimalMax({{prop.exclusiveMaximum() + 1}}, inclusive = false){% endif %}    
    {%- else %}
